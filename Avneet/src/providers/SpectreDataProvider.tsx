...
export function SpectreDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const [onboarding, setOnboarding] = useState<OnboardingAnswers | Record<string, unknown>>(ONBOARDING_ANSWERS as OnboardingAnswers);
  ...
  const refresh = async (): Promise<void> => {
    ...
    try {
      ...
      const [inv, q, g, ob] = await Promise.all([
        ...
        getQueue({ signal }),
        getGraph({ signal }),
        getOnboarding({ signal }).catch(() => ONBOARDING_ANSWERS),
      ]);
      cancel();
